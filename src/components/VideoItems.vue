  ...
  <v-item-group>
    <v-container>
      <v-row>
        ...
        <v-col v-for="(item, key, index) in $store.state.video.items.items" :key="index" cols="12" md="4">
...
export default {
  ...
  data() {
    return {
      videoWidth: 288,
      videoHeight: 162,
      searchKeyword: this.$store.state.selectedHz + 'Hz music' || 'Solfeggio Harmonics Music',
    }
  },
  computed: {
